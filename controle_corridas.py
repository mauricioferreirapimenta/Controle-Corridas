...
if aba == "Adicionar Corrida":
    ...
    melhores_tempos = df.dropna(subset=['Tempo_td']).sort_values(by='Tempo_td').groupby('Distância').first()

    st.subheader("🏅 Melhores Tempos por Distância")
    for dist, row in melhores_tempos.iterrows():
        tempo_formatado = row['Tempo']
        corrida = row['Corrida']
        data = pd.to_datetime(row['Data']).strftime('%d/%m/%Y')
        st.markdown(f"**{int(dist)} km**: {tempo_formatado} — _{corrida}_ em {data}")

    st.download_button(
        label="📥 Baixar dados em Excel",
        data=gerar_excel_download(df.drop(columns='Tempo_td')),
