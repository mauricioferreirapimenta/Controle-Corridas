...
if aba == "Adicionar Corrida":
    ...
    if distancia_selecionada:
        ...
        corridas_na_distancia = df[df['Distância'] == distancia_selecionada]
        melhores_df = corridas_na_distancia.dropna(subset=['Tempo_td'])

        if not corridas_na_distancia.empty and melhores_df.empty:
            st.info("Corridas encontradas para esta distância, mas sem tempos registrados.")
        elif not melhores_df.empty:
