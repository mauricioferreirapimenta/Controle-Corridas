...
def carregar_dados():
    try:
        ...
        df['Data'] = pd.to_datetime(df['Data']).dt.date  # garantir só data, sem hora
...
def salvar_dados(df):
    df_export = df.copy()
    df_export['Data'] = pd.to_datetime(df_export['Data'])  # para salvar corretamente no Excel
...
def gerar_excel_download(df):
    output = BytesIO()
    df_export = df.copy()
    df_export['Data'] = pd.to_datetime(df_export['Data'])  # garantir boa exportação
